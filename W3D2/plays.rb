...
class Play
  ...
  def self.find_by_title(title)
    ...
    WHERE
      title = ?
    SQL
    return nil unless play_title.length > 0
  ...
  def self.find_by_playwright(name)
    ...
    all_plays = PlayDBConnection.instance.execute(<<-SQL,playwright.id)
    SELECT
    *
    FROM
      plays
...
class Playwright
  ...
  #   plays.title
  #   FROM
  #   playwrights JOIN plays ON plays.playwright_id = playwrights.id
  #   SQL
